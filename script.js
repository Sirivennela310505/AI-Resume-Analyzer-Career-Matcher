...
async function uploadResumePDF() {
  ...
  reader.onload = async function () {
    ...
    document.getElementById("uploadStatus").innerHTML = "Resume uploaded successfully. Analysis generated below.";
...
function analyzeResume() {
  ...
  const missing = keys.filter(skill =>
    !text.includes(skill)
  );
  ...
  if (
    ...
    (
      matched.includes("tensorflow") ||
      matched.includes("pytorch")
    )
  ) {
    ...
    <p>
      ${
        matched.length
          ? matched
              .map(skill =>
                `<span class="tag">${skills[skill]}</span>`
              )
              .join(" ")
      ...
      ${
        missing.length
          ? missing
              .slice(0, 8)
              .map(skill =>
                `<span class="tag missing">${skills[skill]}</span>`
              )
              .join(" ")
...
function recommendCareer() {
  ...
  const entry =
    careerPaths[lastJobRole] ||
    careerPaths["Student / Fresher"];
...
function showInsights() {
  ...
  const insightDiv =
    document.getElementById("insightResult");
  ...
  insightDiv.innerHTML = `
    ...
    <ul>
      ...
      <li>
        ...
        <strong>
          ${lastMatchedSkills.slice(0, 5).join(", ")}
        </strong>
      </li>
      ...
      <li>
        Include certifications and technical achievements clearly.
      </li>

      <li>
        Keep resume clean and ATS friendly.
      </li>

      <li>
        Add strong GitHub projects and portfolio links.
      </li>
